...
Tuscany<-read.csv("data/serodata_mev_it_0506.csv",header=T,dec=".",sep=",")
...
#  list("S"=sample(1:K,Nsub,replace=TRUE),
#       "p"=as.numeric(rdirichlet(1,e0)))}

#Fit JAGS
timer <- proc.time()
mv.const.n.4comp.dv<-jags.parallel(mea05.data,parameters.to.save = mea05.params,n.chains=2,
                           model.file="codes/mv_const_n_dv.txt",
                           n.iter=30000,n.thin = 10,n.cluster = 2)
time.taken <- proc.time() - timer
print(timings <- time.taken[3]/60)
...
gelman.test<-gelman.diag(mcmc.list(mcmc(fit$BUGSoutput$sims.matrix[1:1500,c(928:941)]),
                                   mcmc(fit$BUGSoutput$sims.matrix[1501:3000,c(928:941)])))
which(gelman.test$psrf[,"Upper C.I."]>1.01) 
geweke.test<-geweke.diag(mcmc(fit$BUGSoutput$sims.matrix[,c(928:941)]))
which(abs(geweke.test$z)>qnorm(0.975)) 
heidel.test<-heidel.diag(mcmc(fit$BUGSoutput$sims.matrix[,c(928:941)]))
which(heidel.test[,"halfwidth"]/heidel.test[,"mean"]>0.1) 
...
#Fit JAGS
timer <- proc.time()
mv.nonparam.n.4comp.dv<-jags.parallel(mea05.data,parameters.to.save = mea05.params,n.chains=4,
                                      model.file="codes/mv_nonparam_n_dv.txt",
                                      n.iter=30000,n.thin = 10,n.cluster = 4)
time.taken <- proc.time() - timer
print(timings <- time.taken[3]/60)
